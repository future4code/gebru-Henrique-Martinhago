...
localStorage.setItem("Tarefas", JSON.stringify(copiaEstado))
  }

  selectTarefa = (id) => {
const riscaTarefa = this.state.tarefas.map((item)=> {
  if (id === item.id) {
    const riscaTarefa = {
      ...item,
   ...
   this.setState({tarefas: riscaTarefa})
}
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <InputsContainer>
          <input onChange={this.handleChange} type="text" value={this.state.inputValue} onChange={this.onChangeInput}/>
